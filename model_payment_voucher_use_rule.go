/*
支付宝开放平台API

支付宝开放平台v3协议文档

API version: 2024-07-05
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package alipay

import (
	"encoding/json"
)

// checks if the PaymentVoucherUseRule type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PaymentVoucherUseRule{}

// PaymentVoucherUseRule struct for PaymentVoucherUseRule
type PaymentVoucherUseRule struct {
	// 可核销的支付宝小程序id  限制：  1、必须是支付宝小程序 2、如果包含重复的小程序id会自动进行去重操作。
	AvailableAppIds   []string                         `json:"available_app_ids,omitempty"`
	AvailableGoods    *PaymentVoucherAvailableGoods    `json:"available_goods,omitempty"`
	AvailableMerchant *PaymentVoucherAvailableMerchant `json:"available_merchant,omitempty"`
	// 可核销支付门店id。 限制：  1、available_store_ids中的门店id必须是支付宝门店id。  2、available_store_ids如果包含重复的门店id会自动进行去重操作。
	AvailableStoreIds []string           `json:"available_store_ids,omitempty"`
	FixVoucher        *PaymentFixVoucher `json:"fix_voucher,omitempty"`
	// 不可核销商品编码，商家自定义商品编码。当用户支付时，交易中的商品编码和支付券配置的商品编码有任一匹配时，不可以使用优惠券。  限制: 1、available_goods与unavailable_goods_ids只能二选一或者同时为空
	UnavailableGoodsIds []string `json:"unavailable_goods_ids,omitempty"`
	// 核销跳转方式,默认CAN_USE(详情页展示可使用按钮)
	UseMode *string `json:"use_mode,omitempty"`
	// 核销跳转链接 限制: 1、只有use_mode为SELF_LINK时该值有效且必传。 2、核销跳转链接必须是支付宝小程序链接。
	UseUrl *string `json:"use_url,omitempty"`
	// 每个支付宝账号的核销次数限制。 默认不限制
	VoucherQuantityLimitPerUser *int32 `json:"voucher_quantity_limit_per_user,omitempty"`
	// 核销次数限制周期配置。默认值LIFE_CYCLE表示限制每个支付宝账号在整个活动期间核销次数。
	VoucherQuantityLimitPerUserPeriodType *string                    `json:"voucher_quantity_limit_per_user_period_type,omitempty"`
	VoucherValidPeriod                    *PaymentVoucherValidPeriod `json:"voucher_valid_period,omitempty"`
}

// NewPaymentVoucherUseRule instantiates a new PaymentVoucherUseRule object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPaymentVoucherUseRule() *PaymentVoucherUseRule {
	this := PaymentVoucherUseRule{}
	return &this
}

// NewPaymentVoucherUseRuleWithDefaults instantiates a new PaymentVoucherUseRule object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPaymentVoucherUseRuleWithDefaults() *PaymentVoucherUseRule {
	this := PaymentVoucherUseRule{}
	return &this
}

// GetAvailableAppIds returns the AvailableAppIds field value if set, zero value otherwise.
func (o *PaymentVoucherUseRule) GetAvailableAppIds() []string {
	if o == nil || IsNil(o.AvailableAppIds) {
		var ret []string
		return ret
	}
	return o.AvailableAppIds
}

// GetAvailableAppIdsOk returns a tuple with the AvailableAppIds field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaymentVoucherUseRule) GetAvailableAppIdsOk() ([]string, bool) {
	if o == nil || IsNil(o.AvailableAppIds) {
		return nil, false
	}
	return o.AvailableAppIds, true
}

// HasAvailableAppIds returns a boolean if a field has been set.
func (o *PaymentVoucherUseRule) HasAvailableAppIds() bool {
	if o != nil && !IsNil(o.AvailableAppIds) {
		return true
	}

	return false
}

// SetAvailableAppIds gets a reference to the given []string and assigns it to the AvailableAppIds field.
func (o *PaymentVoucherUseRule) SetAvailableAppIds(v []string) {
	o.AvailableAppIds = v
}

// GetAvailableGoods returns the AvailableGoods field value if set, zero value otherwise.
func (o *PaymentVoucherUseRule) GetAvailableGoods() PaymentVoucherAvailableGoods {
	if o == nil || IsNil(o.AvailableGoods) {
		var ret PaymentVoucherAvailableGoods
		return ret
	}
	return *o.AvailableGoods
}

// GetAvailableGoodsOk returns a tuple with the AvailableGoods field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaymentVoucherUseRule) GetAvailableGoodsOk() (*PaymentVoucherAvailableGoods, bool) {
	if o == nil || IsNil(o.AvailableGoods) {
		return nil, false
	}
	return o.AvailableGoods, true
}

// HasAvailableGoods returns a boolean if a field has been set.
func (o *PaymentVoucherUseRule) HasAvailableGoods() bool {
	if o != nil && !IsNil(o.AvailableGoods) {
		return true
	}

	return false
}

// SetAvailableGoods gets a reference to the given PaymentVoucherAvailableGoods and assigns it to the AvailableGoods field.
func (o *PaymentVoucherUseRule) SetAvailableGoods(v PaymentVoucherAvailableGoods) {
	o.AvailableGoods = &v
}

// GetAvailableMerchant returns the AvailableMerchant field value if set, zero value otherwise.
func (o *PaymentVoucherUseRule) GetAvailableMerchant() PaymentVoucherAvailableMerchant {
	if o == nil || IsNil(o.AvailableMerchant) {
		var ret PaymentVoucherAvailableMerchant
		return ret
	}
	return *o.AvailableMerchant
}

// GetAvailableMerchantOk returns a tuple with the AvailableMerchant field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaymentVoucherUseRule) GetAvailableMerchantOk() (*PaymentVoucherAvailableMerchant, bool) {
	if o == nil || IsNil(o.AvailableMerchant) {
		return nil, false
	}
	return o.AvailableMerchant, true
}

// HasAvailableMerchant returns a boolean if a field has been set.
func (o *PaymentVoucherUseRule) HasAvailableMerchant() bool {
	if o != nil && !IsNil(o.AvailableMerchant) {
		return true
	}

	return false
}

// SetAvailableMerchant gets a reference to the given PaymentVoucherAvailableMerchant and assigns it to the AvailableMerchant field.
func (o *PaymentVoucherUseRule) SetAvailableMerchant(v PaymentVoucherAvailableMerchant) {
	o.AvailableMerchant = &v
}

// GetAvailableStoreIds returns the AvailableStoreIds field value if set, zero value otherwise.
func (o *PaymentVoucherUseRule) GetAvailableStoreIds() []string {
	if o == nil || IsNil(o.AvailableStoreIds) {
		var ret []string
		return ret
	}
	return o.AvailableStoreIds
}

// GetAvailableStoreIdsOk returns a tuple with the AvailableStoreIds field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaymentVoucherUseRule) GetAvailableStoreIdsOk() ([]string, bool) {
	if o == nil || IsNil(o.AvailableStoreIds) {
		return nil, false
	}
	return o.AvailableStoreIds, true
}

// HasAvailableStoreIds returns a boolean if a field has been set.
func (o *PaymentVoucherUseRule) HasAvailableStoreIds() bool {
	if o != nil && !IsNil(o.AvailableStoreIds) {
		return true
	}

	return false
}

// SetAvailableStoreIds gets a reference to the given []string and assigns it to the AvailableStoreIds field.
func (o *PaymentVoucherUseRule) SetAvailableStoreIds(v []string) {
	o.AvailableStoreIds = v
}

// GetFixVoucher returns the FixVoucher field value if set, zero value otherwise.
func (o *PaymentVoucherUseRule) GetFixVoucher() PaymentFixVoucher {
	if o == nil || IsNil(o.FixVoucher) {
		var ret PaymentFixVoucher
		return ret
	}
	return *o.FixVoucher
}

// GetFixVoucherOk returns a tuple with the FixVoucher field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaymentVoucherUseRule) GetFixVoucherOk() (*PaymentFixVoucher, bool) {
	if o == nil || IsNil(o.FixVoucher) {
		return nil, false
	}
	return o.FixVoucher, true
}

// HasFixVoucher returns a boolean if a field has been set.
func (o *PaymentVoucherUseRule) HasFixVoucher() bool {
	if o != nil && !IsNil(o.FixVoucher) {
		return true
	}

	return false
}

// SetFixVoucher gets a reference to the given PaymentFixVoucher and assigns it to the FixVoucher field.
func (o *PaymentVoucherUseRule) SetFixVoucher(v PaymentFixVoucher) {
	o.FixVoucher = &v
}

// GetUnavailableGoodsIds returns the UnavailableGoodsIds field value if set, zero value otherwise.
func (o *PaymentVoucherUseRule) GetUnavailableGoodsIds() []string {
	if o == nil || IsNil(o.UnavailableGoodsIds) {
		var ret []string
		return ret
	}
	return o.UnavailableGoodsIds
}

// GetUnavailableGoodsIdsOk returns a tuple with the UnavailableGoodsIds field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaymentVoucherUseRule) GetUnavailableGoodsIdsOk() ([]string, bool) {
	if o == nil || IsNil(o.UnavailableGoodsIds) {
		return nil, false
	}
	return o.UnavailableGoodsIds, true
}

// HasUnavailableGoodsIds returns a boolean if a field has been set.
func (o *PaymentVoucherUseRule) HasUnavailableGoodsIds() bool {
	if o != nil && !IsNil(o.UnavailableGoodsIds) {
		return true
	}

	return false
}

// SetUnavailableGoodsIds gets a reference to the given []string and assigns it to the UnavailableGoodsIds field.
func (o *PaymentVoucherUseRule) SetUnavailableGoodsIds(v []string) {
	o.UnavailableGoodsIds = v
}

// GetUseMode returns the UseMode field value if set, zero value otherwise.
func (o *PaymentVoucherUseRule) GetUseMode() string {
	if o == nil || IsNil(o.UseMode) {
		var ret string
		return ret
	}
	return *o.UseMode
}

// GetUseModeOk returns a tuple with the UseMode field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaymentVoucherUseRule) GetUseModeOk() (*string, bool) {
	if o == nil || IsNil(o.UseMode) {
		return nil, false
	}
	return o.UseMode, true
}

// HasUseMode returns a boolean if a field has been set.
func (o *PaymentVoucherUseRule) HasUseMode() bool {
	if o != nil && !IsNil(o.UseMode) {
		return true
	}

	return false
}

// SetUseMode gets a reference to the given string and assigns it to the UseMode field.
func (o *PaymentVoucherUseRule) SetUseMode(v string) {
	o.UseMode = &v
}

// GetUseUrl returns the UseUrl field value if set, zero value otherwise.
func (o *PaymentVoucherUseRule) GetUseUrl() string {
	if o == nil || IsNil(o.UseUrl) {
		var ret string
		return ret
	}
	return *o.UseUrl
}

// GetUseUrlOk returns a tuple with the UseUrl field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaymentVoucherUseRule) GetUseUrlOk() (*string, bool) {
	if o == nil || IsNil(o.UseUrl) {
		return nil, false
	}
	return o.UseUrl, true
}

// HasUseUrl returns a boolean if a field has been set.
func (o *PaymentVoucherUseRule) HasUseUrl() bool {
	if o != nil && !IsNil(o.UseUrl) {
		return true
	}

	return false
}

// SetUseUrl gets a reference to the given string and assigns it to the UseUrl field.
func (o *PaymentVoucherUseRule) SetUseUrl(v string) {
	o.UseUrl = &v
}

// GetVoucherQuantityLimitPerUser returns the VoucherQuantityLimitPerUser field value if set, zero value otherwise.
func (o *PaymentVoucherUseRule) GetVoucherQuantityLimitPerUser() int32 {
	if o == nil || IsNil(o.VoucherQuantityLimitPerUser) {
		var ret int32
		return ret
	}
	return *o.VoucherQuantityLimitPerUser
}

// GetVoucherQuantityLimitPerUserOk returns a tuple with the VoucherQuantityLimitPerUser field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaymentVoucherUseRule) GetVoucherQuantityLimitPerUserOk() (*int32, bool) {
	if o == nil || IsNil(o.VoucherQuantityLimitPerUser) {
		return nil, false
	}
	return o.VoucherQuantityLimitPerUser, true
}

// HasVoucherQuantityLimitPerUser returns a boolean if a field has been set.
func (o *PaymentVoucherUseRule) HasVoucherQuantityLimitPerUser() bool {
	if o != nil && !IsNil(o.VoucherQuantityLimitPerUser) {
		return true
	}

	return false
}

// SetVoucherQuantityLimitPerUser gets a reference to the given int32 and assigns it to the VoucherQuantityLimitPerUser field.
func (o *PaymentVoucherUseRule) SetVoucherQuantityLimitPerUser(v int32) {
	o.VoucherQuantityLimitPerUser = &v
}

// GetVoucherQuantityLimitPerUserPeriodType returns the VoucherQuantityLimitPerUserPeriodType field value if set, zero value otherwise.
func (o *PaymentVoucherUseRule) GetVoucherQuantityLimitPerUserPeriodType() string {
	if o == nil || IsNil(o.VoucherQuantityLimitPerUserPeriodType) {
		var ret string
		return ret
	}
	return *o.VoucherQuantityLimitPerUserPeriodType
}

// GetVoucherQuantityLimitPerUserPeriodTypeOk returns a tuple with the VoucherQuantityLimitPerUserPeriodType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaymentVoucherUseRule) GetVoucherQuantityLimitPerUserPeriodTypeOk() (*string, bool) {
	if o == nil || IsNil(o.VoucherQuantityLimitPerUserPeriodType) {
		return nil, false
	}
	return o.VoucherQuantityLimitPerUserPeriodType, true
}

// HasVoucherQuantityLimitPerUserPeriodType returns a boolean if a field has been set.
func (o *PaymentVoucherUseRule) HasVoucherQuantityLimitPerUserPeriodType() bool {
	if o != nil && !IsNil(o.VoucherQuantityLimitPerUserPeriodType) {
		return true
	}

	return false
}

// SetVoucherQuantityLimitPerUserPeriodType gets a reference to the given string and assigns it to the VoucherQuantityLimitPerUserPeriodType field.
func (o *PaymentVoucherUseRule) SetVoucherQuantityLimitPerUserPeriodType(v string) {
	o.VoucherQuantityLimitPerUserPeriodType = &v
}

// GetVoucherValidPeriod returns the VoucherValidPeriod field value if set, zero value otherwise.
func (o *PaymentVoucherUseRule) GetVoucherValidPeriod() PaymentVoucherValidPeriod {
	if o == nil || IsNil(o.VoucherValidPeriod) {
		var ret PaymentVoucherValidPeriod
		return ret
	}
	return *o.VoucherValidPeriod
}

// GetVoucherValidPeriodOk returns a tuple with the VoucherValidPeriod field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaymentVoucherUseRule) GetVoucherValidPeriodOk() (*PaymentVoucherValidPeriod, bool) {
	if o == nil || IsNil(o.VoucherValidPeriod) {
		return nil, false
	}
	return o.VoucherValidPeriod, true
}

// HasVoucherValidPeriod returns a boolean if a field has been set.
func (o *PaymentVoucherUseRule) HasVoucherValidPeriod() bool {
	if o != nil && !IsNil(o.VoucherValidPeriod) {
		return true
	}

	return false
}

// SetVoucherValidPeriod gets a reference to the given PaymentVoucherValidPeriod and assigns it to the VoucherValidPeriod field.
func (o *PaymentVoucherUseRule) SetVoucherValidPeriod(v PaymentVoucherValidPeriod) {
	o.VoucherValidPeriod = &v
}

func (o PaymentVoucherUseRule) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PaymentVoucherUseRule) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.AvailableAppIds) {
		toSerialize["available_app_ids"] = o.AvailableAppIds
	}
	if !IsNil(o.AvailableGoods) {
		toSerialize["available_goods"] = o.AvailableGoods
	}
	if !IsNil(o.AvailableMerchant) {
		toSerialize["available_merchant"] = o.AvailableMerchant
	}
	if !IsNil(o.AvailableStoreIds) {
		toSerialize["available_store_ids"] = o.AvailableStoreIds
	}
	if !IsNil(o.FixVoucher) {
		toSerialize["fix_voucher"] = o.FixVoucher
	}
	if !IsNil(o.UnavailableGoodsIds) {
		toSerialize["unavailable_goods_ids"] = o.UnavailableGoodsIds
	}
	if !IsNil(o.UseMode) {
		toSerialize["use_mode"] = o.UseMode
	}
	if !IsNil(o.UseUrl) {
		toSerialize["use_url"] = o.UseUrl
	}
	if !IsNil(o.VoucherQuantityLimitPerUser) {
		toSerialize["voucher_quantity_limit_per_user"] = o.VoucherQuantityLimitPerUser
	}
	if !IsNil(o.VoucherQuantityLimitPerUserPeriodType) {
		toSerialize["voucher_quantity_limit_per_user_period_type"] = o.VoucherQuantityLimitPerUserPeriodType
	}
	if !IsNil(o.VoucherValidPeriod) {
		toSerialize["voucher_valid_period"] = o.VoucherValidPeriod
	}
	return toSerialize, nil
}

type NullablePaymentVoucherUseRule struct {
	value *PaymentVoucherUseRule
	isSet bool
}

func (v NullablePaymentVoucherUseRule) Get() *PaymentVoucherUseRule {
	return v.value
}

func (v *NullablePaymentVoucherUseRule) Set(val *PaymentVoucherUseRule) {
	v.value = val
	v.isSet = true
}

func (v NullablePaymentVoucherUseRule) IsSet() bool {
	return v.isSet
}

func (v *NullablePaymentVoucherUseRule) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePaymentVoucherUseRule(val *PaymentVoucherUseRule) *NullablePaymentVoucherUseRule {
	return &NullablePaymentVoucherUseRule{value: val, isSet: true}
}

func (v NullablePaymentVoucherUseRule) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePaymentVoucherUseRule) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
